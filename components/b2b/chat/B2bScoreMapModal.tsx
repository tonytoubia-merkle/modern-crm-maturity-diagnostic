...
interface B2bScoreMapModalProps {
...
function ScoreRow({
  ...
  qId: string;
  text: string;
  score: B2bInferredScore | undefined;
  isSkipped: boolean;
          ...
          <>
            ...
            {score && (
              <span className="text-[10px] text-slate-500 ml-0.5">
                {B2B_SCORE_LABELS[Math.round(score.score)]}
              </span>
            )}
          </>
        )}
...
export function B2bScoreMapModal({
  scores,
  skipped,
  industry,
  onClose,
  ...
  allCovered,
  clientFacing = false,
}: B2bScoreMapModalProps) {
  const industryQuestions = industry
    ? B2B_INDUSTRY_QUESTIONS.filter((q) => q.industry === industry)
    : [];
      ...
      <div className="relative bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-2xl mx-4 p-6">
        ...
        <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-2">
          {B2B_CAPABILITIES_ORDER.map((cap) => {
            const qs = B2B_CORE_QUESTIONS.filter((q) => q.capability === cap);
            return (
              <div key={cap}>
                <p
