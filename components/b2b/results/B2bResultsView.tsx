...
import { B2bMaturityStageCard } from "./B2bMaturityStageCard";
import { B2bCapabilityHeatmap } from "./B2bCapabilityHeatmap";
import { B2bOpportunityCards } from "./B2bOpportunityCards";
import { B2bSalesforceOutput } from "./B2bSalesforceOutput";
...
interface B2bResultsViewProps {
  results: B2bDiagnosticResults;
  shareId: string;
  responses?: B2bResponseItem[];
}

const ALL_QUESTIONS = [...B2B_CORE_QUESTIONS, ...B2B_INDUSTRY_QUESTIONS];
...
export function B2bResultsView({
  results,
  shareId,
  responses = [],
}: B2bResultsViewProps) {
  ...
  const [viewMode, setViewMode] = useState<B2bViewMode>("internal");
  ...
  const shareUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/b2b/results/${shareId}`
      : `/b2b/results/${shareId}`;
      ...
      <div className="max-w-4xl mx-auto px-4 py-10" ref={printRef}>
        ...
        <div className="mb-8 print:mb-6">
          ...
          <p className="text-slate-500 mt-1">
            ...
            {assessment.industry &&
              B2B_INDUSTRY_LABELS[assessment.industry] &&
              ` · ${B2B_INDUSTRY_LABELS[assessment.industry]}`}
          </p>
        ...
        {!isClient && (
          <Card className="mb-6 print:hidden">
            ...
            <CardContent>
              <B2bSalesforceOutput results={results} />
            </CardContent>
          </Card>
        )}
