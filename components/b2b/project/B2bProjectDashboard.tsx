...
import { B2bCapabilityHeatmap } from "@/components/b2b/results/B2bCapabilityHeatmap";
import { B2bStakeholderManager } from "./B2bStakeholderManager";
import {
...
import { getB2bSmeForOpportunity } from "@/lib/b2b/data/smeMapping";
import type {
  B2bCapability,
  B2bCapabilityScore,
  B2bWorkshopAgenda,
...
interface ProjectData {
  ...
  workshop_agenda: B2bWorkshopAgenda | null;
}

interface B2bProjectDashboardProps {
  projectShareId: string;
}

export function B2bProjectDashboard({
  projectShareId,
}: B2bProjectDashboardProps) {
  ...
  const fetchData = useCallback(async () => {
    const res = await fetch(`/api/b2b/projects/${projectShareId}`);
    if (!res.ok) return;
  ...
  const handleAggregate = async () => {
    ...
    setAggregating(true);
    try {
      const res = await fetch(`/api/b2b/projects/${project.id}/aggregate`, {
        method: "POST",
      });
  ...
  const copyLink = (token: string) => {
    navigator.clipboard.writeText(`${baseUrl}/b2b/survey/${token}`);
    setCopied(token);
  ...
  const capabilityScores: B2bCapabilityScore[] = project.aggregated_scores
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          ...
          {project.industry &&
            ` · ${B2B_INDUSTRY_LABELS[project.industry as keyof typeof B2B_INDUSTRY_LABELS] ?? project.industry}`}
      ...
      {project.aggregated_scores && (
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <B2bCapabilityHeatmap scores={capabilityScores} />
        </div>
      )}
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <div className="space-y-2">
            {triggeredOpps.map((opp) => {
              const smes = getB2bSmeForOpportunity(opp.id);
              return (
                <div
                  key={opp.id}
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    ...
                    {opp.capabilities.map((c: B2bCapability) => (
                      <span
                        key={c}
                        className="text-[10px] text-slate-400"
                      >
                        · {B2B_CAPABILITY_LABELS[c]}
                      </span>
                    ))}
                  </div>
                  {smes.length > 0 && (
      ...
      {project.workshop_agenda && (
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <p className="text-[11px] text-slate-400 mt-4">
            ...
            <span className="font-mono">lib/b2b/data/vignettes.ts</span> with
