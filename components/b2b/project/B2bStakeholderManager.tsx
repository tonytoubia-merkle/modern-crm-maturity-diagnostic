...
interface B2bStakeholderManagerProps {
  projectId: string;
  onDone: () => void;
}

export function B2bStakeholderManager({
  projectId,
  onDone,
}: B2bStakeholderManagerProps) {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/b2b/projects/${projectId}/stakeholders`, {
        method: "POST",
  ...
  const copyLink = (token: string) => {
    const url = `${baseUrl}/b2b/survey/${token}`;
