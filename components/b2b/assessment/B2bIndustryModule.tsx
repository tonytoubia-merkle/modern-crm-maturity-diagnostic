...
import type { B2bIndustry, B2bResponseItem } from "@/lib/b2b/types";

interface B2bIndustryModuleProps {
  ...
  onComplete: (selectedIndustry: B2bIndustry | null) => void;
  onSkip: () => void;
  preSelectedIndustry?: B2bIndustry | null;
}

export function B2bIndustryModule({
  responses,
  onScore,
  onNotes,
  onRemoveResponse,
...
}: B2bIndustryModuleProps) {
  const [selectedIndustry, setSelectedIndustry] = useState<B2bIndustry | null>(
    preSelectedIndustry
  );
  ...
  const industryQuestions = selectedIndustry
    ? B2B_INDUSTRY_QUESTIONS.filter((q) => q.industry === selectedIndustry)
    : [];
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {(Object.keys(B2B_INDUSTRY_LABELS) as B2bIndustry[]).map((ind) => (
            <button
              key={ind}
              type="button"
        ...
        <div className="flex gap-3 pt-2">
          {selectedIndustry ? (
            <Button onClick={() => setShowQuestions(true)}>
              Continue with {B2B_INDUSTRY_LABELS[selectedIndustry]}
            </Button>
          ) : null}
    ...
    <div className="space-y-6">
      <div>
        ...
        <h2 className="text-2xl font-bold text-slate-900 mt-1">
          {selectedIndustry ? B2B_INDUSTRY_LABELS[selectedIndustry] : ""}
        </h2>
        <p className="mt-2 text-slate-600 text-sm leading-relaxed">
