...
import { B2bSetupForm, type B2bSetupData } from "./B2bSetupForm";
import { B2bCapabilitySection } from "./B2bCapabilitySection";
import { B2bIndustryModule } from "./B2bIndustryModule";
...
import type { B2bCapability, B2bIndustry, B2bResponseItem } from "@/lib/b2b/types";

const TOTAL_CORE_STEPS = 6;

interface B2bAssessmentFlowProps {
...
export function B2bAssessmentFlow({
  ...
  const handleScore = useCallback(
    (
      ...
      score: number,
      capability: B2bCapability | string,
      isIndustryQuestion = false
    ) => {
      setResponses((prev) => {
        const existing = prev.findIndex((r) => r.questionId === questionId);
        const item: B2bResponseItem = {
          questionId,
          score,
          capability: capability as B2bCapability,
          isIndustryQuestion,
  ...
  const saveCurrentResponses = async () => {
    ...
    setSaving(true);
    try {
      await fetch(`/api/b2b/assessments/${assessmentId}/responses`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleComplete = async (selectedIndustry: B2bIndustry | null) => {
    ...
    try {
      ...
      await fetch(`/api/b2b/assessments/${assessmentId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          status: "completed",
      ...
      if (stakeholderId) {
        await fetch(`/api/b2b/stakeholders/${stakeholderId}/complete`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      window.location.href = `/b2b/results/${shareId}`;
    } catch (e) {
    ...
    <div className="min-h-screen font-merkle bg-merkle-grey-60">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
          {step === 0 && <B2bSetupForm onSubmit={handleSetup} />}

          {step >= 1 && step <= TOTAL_CORE_STEPS && currentCapability && (
            <div className="space-y-8">
              <B2bCapabilitySection
