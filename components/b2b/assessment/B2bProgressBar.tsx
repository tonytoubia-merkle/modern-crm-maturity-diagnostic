...
export function B2bProgressBar({
  currentStep,
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center gap-1 mb-3 overflow-x-auto pb-1">
        ...
        {B2B_CAPABILITIES_ORDER.map((cap, idx) => {
          const stepNum = idx + 1;
          ...
          return (
            <div key={cap} className="flex items-center gap-1">
              ...
              <div
                ...
                title={B2B_CAPABILITY_LABELS[cap]}
              >
                {done ? "✓" : stepNum}
              </div>
            </div>
        ...
        {hasIndustry && (
          <div className="flex items-center gap-1">
            ...
            <div
              className={cn(
                "flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-colors",
                currentStep > B2B_CAPABILITIES_ORDER.length + 1
                  ? "bg-blue-600 border-blue-600 text-white"
                  : currentStep === B2B_CAPABILITIES_ORDER.length + 1
                  ? "bg-white border-blue-600 text-blue-600"
                  : "bg-white border-slate-200 text-slate-400"
            ...
            >
              {currentStep > B2B_CAPABILITIES_ORDER.length + 1
                ? "✓"
                : String(B2B_CAPABILITIES_ORDER.length + 1)}
            </div>
          </div>
        )}
      </div>
