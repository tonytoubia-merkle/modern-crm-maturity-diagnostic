...
import { CscMaturityStageCard } from "./CscMaturityStageCard";
import { CscCapabilityHeatmap } from "./CscCapabilityHeatmap";
import { CscOpportunityCards } from "./CscOpportunityCards";
import { CscSalesforceOutput } from "./CscSalesforceOutput";
...
interface CscResultsViewProps {
  results: CscDiagnosticResults;
  shareId: string;
  responses?: CscResponseItem[];
}

const ALL_QUESTIONS = [...CSC_CORE_QUESTIONS, ...CSC_INDUSTRY_QUESTIONS];
...
export function CscResultsView({
  results,
  shareId,
  responses = [],
}: CscResultsViewProps) {
  ...
  const [viewMode, setViewMode] = useState<CscViewMode>("internal");
  ...
  const shareUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/csc/results/${shareId}`
      : `/csc/results/${shareId}`;
      ...
      <div className="max-w-4xl mx-auto px-4 py-10" ref={printRef}>
        ...
        <div className="mb-8 print:mb-6">
          ...
          <p className="text-slate-500 mt-1">
            ...
            {assessment.industry &&
              CSC_INDUSTRY_LABELS[assessment.industry] &&
              ` · ${CSC_INDUSTRY_LABELS[assessment.industry]}`}
          </p>
        ...
        {!isClient && (
          <Card className="mb-6 print:hidden">
            ...
            <CardContent>
              <CscSalesforceOutput results={results} />
            </CardContent>
          </Card>
        )}
