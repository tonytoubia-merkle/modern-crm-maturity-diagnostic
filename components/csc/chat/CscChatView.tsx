...
import { CscCoverageTracker } from "./CscCoverageTracker";
import { CscScoreMapModal } from "./CscScoreMapModal";
import { parseCscAssistantMessage } from "@/lib/csc/chat/parseScores";
import {
  calculateCscPhase,
  getCscTotalQuestionCount,
} from "@/lib/csc/chat/coverageUtils";
import {
  CSC_CORE_QUESTIONS,
...
import type { CscIndustry, CscCapability } from "@/lib/csc/types";
...
export function CscChatView({
  ...
  const [messages, setMessages] = useState<CscChatMessage[]>([]);
  const [input, setInput] = useState("");
  const [streaming, setStreaming] = useState(false);
  const [scores, setScores] = useState<Map<string, CscInferredScore>>(
    new Map()
  );
  ...
  const sendMessage = useCallback(
    async (userText: string, isAutoStart = false) => {
      ...
      const scoresObj: Record<string, CscInferredScore> = {};
      scores.forEach((v, k) => {
        scoresObj[k] = v;
      });
      ...
      try {
        ...
        const res = await fetch("/api/csc/chat", {
          method: "POST",
            ...
            try {
              ...
              if (parsed.text) {
                ...
                const { displayContent } =
                  parseCscAssistantMessage(fullResponse);
                setMessages((prev) =>
                  prev.map((m) =>
                    m.id === assistantId
        ...
        if (scoreUpdate) {
          setScores((prev) => {
            ...
            for (const update of scoreUpdate.updates) {
              ...
              let capability: CscCapability = "strategy_planning";
              let isIndustry = false;
              const coreQ = CSC_CORE_QUESTIONS.find(
              ...
              if (coreQ) {
                ...
                isIndustry = true;
                capability = CSC_CAPABILITIES_ORDER[0];
              }
        ...
        console.error("CSC chat error:", err);
        setMessages((prev) => [
          ...prev,
          {
            id: `error-${Date.now()}`,
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/csc/chat/confirm", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          assessmentId,
      ...
      window.location.href = `/csc/results/${data.shareId || shareId}`;
    } catch (err) {
      console.error("CSC confirm error:", err);
      setConfirming(false);
    }
  };
  ...
  const handleUpdateScore = (questionId: string, score: number) => {
    setScores((prev) => {
      ...
      if (existing) {
        ...
        const q = CSC_CORE_QUESTIONS.find((q) => String(q.id) === questionId);
        next.set(questionId, {
          ...
          capability: q?.capability || "strategy_planning",
