...
interface CscScoreMapModalProps {
...
function ScoreRow({
  ...
  qId: string;
  text: string;
  score: CscInferredScore | undefined;
  isSkipped: boolean;
          ...
          <>
            ...
            {score && (
              <span className="text-[10px] text-slate-500 ml-0.5">
                {CSC_SCORE_LABELS[Math.round(score.score)]}
              </span>
            )}
          </>
        )}
...
export function CscScoreMapModal({
  scores,
  skipped,
  industry,
  onClose,
  ...
  allCovered,
  clientFacing = false,
}: CscScoreMapModalProps) {
  const industryQuestions = industry
    ? CSC_INDUSTRY_QUESTIONS.filter((q) => q.industry === industry)
    : [];
      ...
      <div className="relative bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-2xl mx-4 p-6">
        ...
        <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-2">
          {CSC_CAPABILITIES_ORDER.map((cap) => {
            const qs = CSC_CORE_QUESTIONS.filter((q) => q.capability === cap);
            return (
              <div key={cap}>
                <p
