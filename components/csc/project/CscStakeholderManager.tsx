...
interface CscStakeholderManagerProps {
  projectId: string;
  onDone: () => void;
}

export function CscStakeholderManager({
  projectId,
  onDone,
}: CscStakeholderManagerProps) {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/csc/projects/${projectId}/stakeholders`, {
        method: "POST",
  ...
  const copyLink = (token: string) => {
    const url = `${baseUrl}/csc/survey/${token}`;
