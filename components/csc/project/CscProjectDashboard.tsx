...
import { CscCapabilityHeatmap } from "@/components/csc/results/CscCapabilityHeatmap";
import { CscStakeholderManager } from "./CscStakeholderManager";
import {
...
import { getCscSmeForOpportunity } from "@/lib/csc/data/smeMapping";
import type {
  CscCapability,
  CscCapabilityScore,
  CscWorkshopAgenda,
...
interface ProjectData {
  ...
  workshop_agenda: CscWorkshopAgenda | null;
}

interface CscProjectDashboardProps {
  projectShareId: string;
}

export function CscProjectDashboard({
  projectShareId,
}: CscProjectDashboardProps) {
  ...
  const fetchData = useCallback(async () => {
    const res = await fetch(`/api/csc/projects/${projectShareId}`);
    if (!res.ok) return;
  ...
  const handleAggregate = async () => {
    ...
    setAggregating(true);
    try {
      const res = await fetch(`/api/csc/projects/${project.id}/aggregate`, {
        method: "POST",
      });
  ...
  const copyLink = (token: string) => {
    navigator.clipboard.writeText(`${baseUrl}/csc/survey/${token}`);
    setCopied(token);
  ...
  const capabilityScores: CscCapabilityScore[] = project.aggregated_scores
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          ...
          {project.industry &&
            ` · ${CSC_INDUSTRY_LABELS[project.industry] ?? project.industry}`}
      ...
      {project.aggregated_scores && (
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <CscCapabilityHeatmap scores={capabilityScores} />
        </div>
      )}
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <div className="space-y-2">
            {triggeredOpps.map((opp) => {
              const smes = getCscSmeForOpportunity(opp.id);
              return (
                <div
                  key={opp.id}
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    ...
                    {opp.capabilities.map((c: CscCapability) => (
                      <span
                        key={c}
                        className="text-[10px] text-slate-400"
                      >
                        · {CSC_CAPABILITY_LABELS[c]}
                      </span>
                    ))}
                  </div>
                  {smes.length > 0 && (
      ...
      {project.workshop_agenda && (
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <p className="text-[11px] text-slate-400 mt-4">
            ...
            <span className="font-mono">lib/csc/data/vignettes.ts</span> with
