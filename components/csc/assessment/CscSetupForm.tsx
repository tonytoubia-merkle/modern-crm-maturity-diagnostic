...
interface CscSetupFormProps {
  onSubmit: (data: CscSetupData) => Promise<void>;
}

export function CscSetupForm({ onSubmit }: CscSetupFormProps) {
  const [data, setData] = useState<CscSetupData>({
    clientName: "",
    clientCompany: "",
    respondentName: "",
    repEmail: "",
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof CscSetupData, string>>>({});

  const validate = () => {
    const errs: Partial<Record<keyof CscSetupData, string>> = {};
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {(
              Object.entries(CSC_INDUSTRY_LABELS) as [CscIndustry, string][]
            ).map(([key, label]) => (
              <button
                key={key}
                type="button"
