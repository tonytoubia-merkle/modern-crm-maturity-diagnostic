...
import { AientCoverageTracker } from "./AientCoverageTracker";
import { AientScoreMapModal } from "./AientScoreMapModal";
import { parseAientAssistantMessage } from "@/lib/aient/chat/parseScores";
import {
  calculateAientPhase,
  getAientTotalQuestionCount,
} from "@/lib/aient/chat/coverageUtils";
import {
...
import type {
  AientChatMessage,
  AientInferredScore,
} from "@/lib/aient/chat/types";
import type { AientIndustry, AientCapability } from "@/lib/aient/types";
...
export function AientChatView({
  ...
  const [messages, setMessages] = useState<AientChatMessage[]>([]);
  const [input, setInput] = useState("");
  const [streaming, setStreaming] = useState(false);
  const [scores, setScores] = useState<Map<string, AientInferredScore>>(
    new Map()
  );
  ...
  const totalQuestions = getAientTotalQuestionCount(industry);
  const phase = calculateAientPhase(scores, skipped, totalQuestions);
  ...
  const sendMessage = useCallback(
    async (userText: string, isAutoStart = false) => {
      ...
      const scoresObj: Record<string, AientInferredScore> = {};
      scores.forEach((v, k) => {
        scoresObj[k] = v;
      });
      ...
      try {
        ...
        const res = await fetch("/api/aient/chat", {
          method: "POST",
            ...
            try {
              ...
              if (parsed.text) {
                ...
                const { displayContent } =
                  parseAientAssistantMessage(fullResponse);
                setMessages((prev) =>
                  prev.map((m) =>
                    m.id === assistantId
        ...
        if (scoreUpdate) {
          setScores((prev) => {
            ...
            for (const update of scoreUpdate.updates) {
              ...
              let capability: AientCapability = AIENT_CAPABILITIES_ORDER[0];
              let isIndustry = false;
              const coreQ = AIENT_CORE_QUESTIONS.find(
              ...
              if (coreQ) {
                ...
                isIndustry = true;
                capability = AIENT_CAPABILITIES_ORDER[0];
              }
        ...
        console.error("AI for Enterprise chat error:", err);
        setMessages((prev) => [
          ...prev,
          {
            id: `error-${Date.now()}`,
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/aient/chat/confirm", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          assessmentId,
      ...
      window.location.href = `/aient/results/${data.shareId || shareId}`;
    } catch (err) {
      console.error("AI for Enterprise confirm error:", err);
      setConfirming(false);
    }
  };
  ...
  const handleUpdateScore = (questionId: string, score: number) => {
    setScores((prev) => {
      ...
      if (existing) {
        ...
        const q = AIENT_CORE_QUESTIONS.find((q) => String(q.id) === questionId);
        next.set(questionId, {
          ...
          capability: q?.capability || AIENT_CAPABILITIES_ORDER[0],
