...
interface AientScoreMapModalProps {
...
function ScoreRow({
  ...
  qId: string;
  text: string;
  score: AientInferredScore | undefined;
  isSkipped: boolean;
          ...
          <>
            ...
            {score && (
              <span className="text-[10px] text-slate-500 ml-0.5">
                {AIENT_SCORE_LABELS[Math.round(score.score)]}
              </span>
            )}
          </>
        )}
...
export function AientScoreMapModal({
  scores,
  skipped,
  industry,
  onClose,
  ...
  allCovered,
  clientFacing = false,
}: AientScoreMapModalProps) {
  const industryQuestions = industry
    ? AIENT_INDUSTRY_QUESTIONS.filter((q) => q.industry === industry)
    : [];
      ...
      <div className="relative bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-2xl mx-4 p-6">
        ...
        <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-2">
          {AIENT_CAPABILITIES_ORDER.map((cap) => {
            const qs = AIENT_CORE_QUESTIONS.filter((q) => q.capability === cap);
            return (
              <div key={cap}>
                <p
                ...
                {qs.map((q) => (
                  <ScoreRow
                    key={q.id}
                    qId={String(q.id)}
                    text={resolveAientQuestionText(q, industry)}
