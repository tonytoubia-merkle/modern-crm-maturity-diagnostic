...
import type { AientMaturityStage } from "@/lib/aient/types";

interface AientMaturityStageCardProps {
  stage: AientMaturityStage;
  overallScore: number;
}
...
export function AientMaturityStageCard({
  stage,
  overallScore,
}: AientMaturityStageCardProps) {
  const info = AIENT_MATURITY_STAGES[stage];
    ...
    <div className={cn("rounded-xl border p-6", colors.bg, colors.border)}>
      <div className="flex items-start justify-between gap-4 mb-4">
        <div>
          ...
          <h3 className={cn("text-xl font-bold", colors.text)}>{info.label}</h3>
        </div>
        <div
          className={cn(
