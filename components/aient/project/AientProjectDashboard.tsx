...
import { AientCapabilityHeatmap } from "@/components/aient/results/AientCapabilityHeatmap";
import { AientStakeholderManager } from "./AientStakeholderManager";
import {
...
import { getAientSmeForOpportunity } from "@/lib/aient/data/smeMapping";
import type {
  AientCapability,
...
interface ProjectData {
  ...
  workshop_agenda: AientWorkshopAgenda | null;
}

interface AientProjectDashboardProps {
  projectShareId: string;
}

export function AientProjectDashboard({
  projectShareId,
}: AientProjectDashboardProps) {
  ...
  const fetchData = useCallback(async () => {
    const res = await fetch(`/api/aient/projects/${projectShareId}`);
    if (!res.ok) return;
  ...
  const handleAggregate = async () => {
    ...
    setAggregating(true);
    try {
      const res = await fetch(`/api/aient/projects/${project.id}/aggregate`, {
        method: "POST",
      });
  ...
  const copyLink = (token: string) => {
    navigator.clipboard.writeText(`${baseUrl}/aient/survey/${token}`);
    setCopied(token);
  ...
  const capabilityScores: AientCapabilityScore[] = project.aggregated_scores
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          ...
          {project.industry &&
            ` · ${AIENT_INDUSTRY_LABELS[project.industry as keyof typeof AIENT_INDUSTRY_LABELS] ?? project.industry}`}
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <div className="space-y-2">
            {triggeredOpps.map((opp) => {
              const smes = getAientSmeForOpportunity(opp.id);
              return (
                <div
                  key={opp.id}
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    ...
                    {opp.capabilities.map((c: AientCapability) => (
                      <span
                        key={c}
                        className="text-[10px] text-slate-400"
                      >
                        · {AIENT_CAPABILITY_LABELS[c]}
                      </span>
                    ))}
                  </div>
                  {smes.length > 0 && (
      ...
      {project.workshop_agenda && (
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <p className="text-[11px] text-slate-400 mt-4">
            ...
            <span className="font-mono">lib/aient/data/vignettes.ts</span> with
