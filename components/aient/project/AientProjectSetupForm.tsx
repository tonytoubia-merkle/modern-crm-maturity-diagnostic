...
interface AientProjectSetupData {
  clientName: string;
...
interface AientProjectSetupFormProps {
  onSubmit: (data: AientProjectSetupData) => Promise<void>;
}

export function AientProjectSetupForm({ onSubmit }: AientProjectSetupFormProps) {
  const [data, setData] = useState<AientProjectSetupData>({
    clientName: "",
    clientCompany: "",
    industry: "",
    createdByName: "",
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-600 leading-relaxed">
          Set up a AI for Enterprise diagnostic project. Distribute pre-work
        ...
        <p className="mt-1.5 text-xs text-slate-400">
          ...
          <a href="/aient/assessment/new" className="underline hover:text-slate-600">
            Quick Assessment
          </a>{" "}
          instead.
        </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {(Object.entries(AIENT_INDUSTRY_LABELS) as [AientIndustry, string][]).map(
              ([key, label]) => (
                <button
                  key={key}
                  type="button"
