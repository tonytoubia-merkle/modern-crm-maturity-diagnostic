...
import { AientSetupForm, type AientSetupData } from "./AientSetupForm";
import { AientCapabilitySection } from "./AientCapabilitySection";
import { AientIndustryModule } from "./AientIndustryModule";
import { AientProgressBar } from "./AientProgressBar";
...
import type { AientCapability, AientIndustry, AientResponseItem } from "@/lib/aient/types";

const TOTAL_CORE_STEPS = 6;

interface AientAssessmentFlowProps {
  initialAssessmentId?: string | null;
  initialShareId?: string | null;
  initialResponses?: AientResponseItem[];
  initialIndustry?: AientIndustry | null;
  initialStep?: number;
  /** When set, the flow is running inside a stakeholder invite – completion
   *  posts to /api/aient/stakeholders/:id/complete and calls onComplete instead
   *  of redirecting to the results page. */
  stakeholderId?: string | null;
  onComplete?: () => void;
}

export function AientAssessmentFlow({
  ...
  const handleScore = useCallback(
    (
      ...
      score: number,
      capability: AientCapability | string,
      isIndustryQuestion = false
    ) => {
      setResponses((prev) => {
        const existing = prev.findIndex((r) => r.questionId === questionId);
        const item: AientResponseItem = {
          questionId,
          score,
          capability: capability as AientCapability,
          isIndustryQuestion,
  ...
  const saveCurrentResponses = async () => {
    ...
    setSaving(true);
    try {
      await fetch(`/api/aient/assessments/${assessmentId}/responses`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const currentCapability =
    step >= 1 && step <= TOTAL_CORE_STEPS
      ? AIENT_CAPABILITIES_ORDER[step - 1]
      : null;
  ...
  const handleComplete = async (selectedIndustry: AientIndustry | null) => {
    ...
    try {
      ...
      await fetch(`/api/aient/assessments/${assessmentId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          status: "completed",
      ...
      if (stakeholderId) {
        await fetch(`/api/aient/stakeholders/${stakeholderId}/complete`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      window.location.href = `/aient/results/${shareId}`;
    } catch (e) {
    ...
    <div className="min-h-screen font-merkle bg-merkle-grey-60">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
          {step === 0 && <AientSetupForm onSubmit={handleSetup} />}

          {step >= 1 && step <= TOTAL_CORE_STEPS && currentCapability && (
            <div className="space-y-8">
              <AientCapabilitySection
