...
import type { AientIndustry, AientResponseItem } from "@/lib/aient/types";

interface AientIndustryModuleProps {
  ...
  onComplete: (selectedIndustry: AientIndustry | null) => void;
  onSkip: () => void;
  preSelectedIndustry?: AientIndustry | null;
}

export function AientIndustryModule({
  responses,
  onScore,
  onNotes,
  onRemoveResponse,
...
}: AientIndustryModuleProps) {
  const [selectedIndustry, setSelectedIndustry] = useState<AientIndustry | null>(
    preSelectedIndustry
  );
  ...
  const industryQuestions = selectedIndustry
    ? AIENT_INDUSTRY_QUESTIONS.filter((q) => q.industry === selectedIndustry)
    : [];
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {(Object.keys(AIENT_INDUSTRY_LABELS) as AientIndustry[]).map((ind) => (
            <button
              key={ind}
              type="button"
        ...
        <div className="flex gap-3 pt-2">
          {selectedIndustry ? (
            <Button onClick={() => setShowQuestions(true)}>
              Continue with {AIENT_INDUSTRY_LABELS[selectedIndustry]}
            </Button>
          ) : null}
    ...
    <div className="space-y-6">
      <div>
        ...
        <h2 className="text-2xl font-bold text-slate-900 mt-1">
          {selectedIndustry ? AIENT_INDUSTRY_LABELS[selectedIndustry] : ""}
        </h2>
        <p className="mt-2 text-slate-600 text-sm leading-relaxed">
