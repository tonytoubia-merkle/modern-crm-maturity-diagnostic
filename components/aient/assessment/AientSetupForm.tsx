...
export interface AientSetupData {
  clientName: string;
...
interface AientSetupFormProps {
  onSubmit: (data: AientSetupData) => Promise<void>;
}

export function AientSetupForm({ onSubmit }: AientSetupFormProps) {
  const [data, setData] = useState<AientSetupData>({
    clientName: "",
    clientCompany: "",
    respondentName: "",
    repEmail: "",
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof AientSetupData, string>>>({});

  const validate = () => {
    const errs: Partial<Record<keyof AientSetupData, string>> = {};
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {(
              Object.entries(AIENT_INDUSTRY_LABELS) as [AientIndustry, string][]
            ).map(([key, label]) => (
              <button
                key={key}
                type="button"
