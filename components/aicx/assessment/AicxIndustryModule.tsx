...
interface AicxIndustryModuleProps {
  ...
  onComplete: (selectedIndustry: AicxIndustry | null) => void;
  onSkip: () => void;
  preSelectedIndustry?: AicxIndustry | null;
}

export function AicxIndustryModule({
  responses,
  onScore,
  onNotes,
  onRemoveResponse,
...
}: AicxIndustryModuleProps) {
  const [selectedIndustry, setSelectedIndustry] = useState<AicxIndustry | null>(
    preSelectedIndustry
  );
  ...
  const industryQuestions = selectedIndustry
    ? AICX_INDUSTRY_QUESTIONS.filter((q) => q.industry === selectedIndustry)
    : [];
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {(Object.keys(AICX_INDUSTRY_LABELS) as AicxIndustry[]).map((ind) => (
            <button
              key={ind}
              type="button"
        ...
        <div className="flex gap-3 pt-2">
          {selectedIndustry ? (
            <Button onClick={() => setShowQuestions(true)}>
              Continue with {AICX_INDUSTRY_LABELS[selectedIndustry]}
            </Button>
          ) : null}
    ...
    <div className="space-y-6">
      <div>
        ...
        <h2 className="text-2xl font-bold text-slate-900 mt-1">
          {selectedIndustry ? AICX_INDUSTRY_LABELS[selectedIndustry] : ""}
        </h2>
        <p className="mt-2 text-slate-600 text-sm leading-relaxed">
