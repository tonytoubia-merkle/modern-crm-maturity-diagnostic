...
import {
  ...
  AICX_CAPABILITY_DESCRIPTIONS,
  AICX_CAPABILITY_SCOPE_HINTS,
...
interface AicxCapabilitySectionProps {
...
export function AicxCapabilitySection({
  capability,
  responses,
  onScore,
  onNotes,
  ...
  onReadyChange,
  industry,
}: AicxCapabilitySectionProps) {
  const questions = AICX_QUESTIONS_BY_CAPABILITY[capability];
  ...
  useEffect(() => {
    const ready = questions.every(
      (q) =>
        skipped.has(q.id) ||
        responses.find((r) => r.questionId === q.id) !== undefined
    );
    onReadyChange?.(ready);
    ...
    <div className="space-y-6">
      <div>
        <span className="text-xs font-semibold text-blue-600 uppercase tracking-wider">
          {AICX_CAPABILITY_SUBTITLES[capability]}
        </span>
        <h2 className="text-2xl font-bold text-slate-900 mt-0.5">
          {AICX_CAPABILITY_LABELS[capability]}
        </h2>
        <p className="mt-1.5 text-sm text-slate-600 leading-relaxed">
          {AICX_CAPABILITY_DESCRIPTIONS[capability]}
        </p>
        <p className="mt-2 text-xs text-amber-700 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2 leading-relaxed">
          {AICX_CAPABILITY_SCOPE_HINTS[capability]}
        </p>
      </div>
            ...
            >
              <div className="flex gap-3 mb-3">
                ...
                <div className="flex-1">
                  ...
                  >
                    {resolveAicxQuestionText(question, industry)}
                  </p>
