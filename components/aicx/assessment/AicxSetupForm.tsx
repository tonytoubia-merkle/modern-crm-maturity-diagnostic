...
export interface AicxSetupData {
  clientName: string;
...
interface AicxSetupFormProps {
  onSubmit: (data: AicxSetupData) => Promise<void>;
}

export function AicxSetupForm({ onSubmit }: AicxSetupFormProps) {
  const [data, setData] = useState<AicxSetupData>({
    clientName: "",
    clientCompany: "",
    respondentName: "",
    repEmail: "",
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof AicxSetupData, string>>>({});

  const validate = () => {
    const errs: Partial<Record<keyof AicxSetupData, string>> = {};
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-slate-600 text-sm leading-relaxed">
          ...
          personalize your AI for CX diagnostic and makes the report
          easy to share.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {(
              Object.entries(AICX_INDUSTRY_LABELS) as [AicxIndustry, string][]
            ).map(([key, label]) => (
              <button
                key={key}
                type="button"
