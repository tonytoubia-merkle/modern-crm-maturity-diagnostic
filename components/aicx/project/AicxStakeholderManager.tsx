...
interface AicxStakeholderManagerProps {
  projectId: string;
  onDone: () => void;
}

export function AicxStakeholderManager({
  projectId,
  onDone,
}: AicxStakeholderManagerProps) {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/aicx/projects/${projectId}/stakeholders`, {
        method: "POST",
  ...
  const copyLink = (token: string) => {
    const url = `${baseUrl}/aicx/survey/${token}`;
