...
import { AicxMaturityStageCard } from "./AicxMaturityStageCard";
import { AicxCapabilityHeatmap } from "./AicxCapabilityHeatmap";
import { AicxOpportunityCards } from "./AicxOpportunityCards";
import { AicxSalesforceOutput } from "./AicxSalesforceOutput";
...
interface AicxResultsViewProps {
  results: AicxDiagnosticResults;
  shareId: string;
  responses?: AicxResponseItem[];
}

const ALL_QUESTIONS = [...AICX_CORE_QUESTIONS, ...AICX_INDUSTRY_QUESTIONS];
...
export function AicxResultsView({
  results,
  shareId,
  responses = [],
}: AicxResultsViewProps) {
  const {
    assessment,
    capabilityScores,
    overallScore,
  ...
  const [viewMode, setViewMode] = useState<AicxViewMode>("internal");
  ...
  const shareUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/aicx/results/${shareId}`
      : `/aicx/results/${shareId}`;
      ...
      <div className="max-w-4xl mx-auto px-4 py-10" ref={printRef}>
        ...
        <div className="mb-8 print:mb-6">
          ...
          <p className="text-slate-500 mt-1">
            ...
            {assessment.industry &&
              AICX_INDUSTRY_LABELS[assessment.industry] &&
              ` · ${AICX_INDUSTRY_LABELS[assessment.industry]}`}
          </p>
        ...
        {!isClient && (
          <Card className="mb-6 print:hidden">
            ...
            <CardContent>
              <AicxSalesforceOutput results={results} />
            </CardContent>
          </Card>
        )}
