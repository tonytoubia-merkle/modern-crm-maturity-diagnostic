...
import type { AicxDiagnosticResults } from "@/lib/aicx/types";

interface AicxSalesforceOutputProps {
  results: AicxDiagnosticResults;
}

function generateNarrative(results: AicxDiagnosticResults): string {
  const {
    assessment,
    overallScore,
    maturityStage,
  ...
  return `${assessment.clientName} (${assessment.clientCompany}) completed a AI for CX Diagnostic on ${formatDate(
    assessment.createdAt
  )}, scoring ${overallScore.toFixed(
    1
  )} out of 5.0 and placing at ${maturityLabel}. ${stageInfo.description} Key capability gaps were identified in ${
    weakList || "several areas"
  }, representing priority areas for content supply chain transformation. Merkle / dentsu's Content practice is positioned to help ${
    assessment.clientName
  } move from a campaign-shaped content factory to a connected, AI-augmented engine for personalized engagement and growth.`;
}

function generateOpportunityTable(results: AicxDiagnosticResults): string {
  const rows = results.opportunities
    .slice(0, 5)
    .map(
      (opp) =>
        `${opp.title} | ${opp.sfType} | ${
          opp.priority.charAt(0).toUpperCase() + opp.priority.slice(1)
        } | ${opp.engagementSize} | ${opp.valueNarrative.slice(0, 80)}…`
    )
    .join("\n");
...
export function AicxSalesforceOutput({ results }: AicxSalesforceOutputProps) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-slate-500 mb-3">
          Ready to paste into Salesforce account notes or an opportunity
          description.
        </p>
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 mb-3">
          Paste into Salesforce opportunity descriptions, proposal documents, or
          pipeline planning tools.
        </p>
        <div className="relative">
