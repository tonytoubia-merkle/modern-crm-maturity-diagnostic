...
import {
  getAicxScoreBarColor,
  getAicxScoreBgColor,
  formatAicxScore,
} from "@/lib/aicx/scoring";
import { AICX_SCORE_LABELS } from "@/lib/aicx/data/questions";
import type { AicxCapabilityScore } from "@/lib/aicx/types";
...
interface AicxCapabilityHeatmapProps {
              ...
              >
                ...
                <Tooltip
                  formatter={(value: number, name: string) => {
                    ...
                    const label = AICX_SCORE_LABELS[rounded] ?? "";
                    return [
                      `${formatAicxScore(value)}${
                        label ? ` – ${label}` : ""
                      }`,
                      name,
                    ];
        ...
        <div className="flex flex-col">
          ...
          <div className="space-y-3">
            {scores.map((s) => {
              ...
              const scoreLabel =
                AICX_SCORE_LABELS[roundedScore] ??
                AICX_SCORE_LABELS[Math.ceil(s.score)];
              return (
          ...
          {[1, 2, 3, 4, 5].map((v) => (
            <div
              ...
              className={cn(
                ...
                getAicxScoreBgColor(v)
              )}
            >
              <span className="font-bold">{v}</span>
              <span>–</span>
              <span>{AICX_SCORE_LABELS[v]}</span>
            </div>
          ))}
        </div>
      </div>
