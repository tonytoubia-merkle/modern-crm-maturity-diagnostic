...
import { AicxCoverageTracker } from "./AicxCoverageTracker";
import { AicxScoreMapModal } from "./AicxScoreMapModal";
import { parseAicxAssistantMessage } from "@/lib/aicx/chat/parseScores";
import {
  calculateAicxPhase,
  getAicxTotalQuestionCount,
} from "@/lib/aicx/chat/coverageUtils";
import {
  AICX_CORE_QUESTIONS,
...
import type { AicxIndustry, AicxCapability } from "@/lib/aicx/types";
...
export function AicxChatView({
  ...
  const [messages, setMessages] = useState<AicxChatMessage[]>([]);
  const [input, setInput] = useState("");
  const [streaming, setStreaming] = useState(false);
  const [scores, setScores] = useState<Map<string, AicxInferredScore>>(
    new Map()
  );
  ...
  const totalQuestions = getAicxTotalQuestionCount(industry);
  const phase = calculateAicxPhase(scores, skipped, totalQuestions);
  ...
  const sendMessage = useCallback(
    async (userText: string, isAutoStart = false) => {
      ...
      const scoresObj: Record<string, AicxInferredScore> = {};
      scores.forEach((v, k) => {
        scoresObj[k] = v;
      });
      ...
      try {
        ...
        const res = await fetch("/api/aicx/chat", {
          method: "POST",
            ...
            try {
              ...
              if (parsed.text) {
                ...
                const { displayContent } =
                  parseAicxAssistantMessage(fullResponse);
                setMessages((prev) =>
                  prev.map((m) =>
                    m.id === assistantId
        ...
        if (scoreUpdate) {
          setScores((prev) => {
            ...
            for (const update of scoreUpdate.updates) {
              ...
              let capability: AicxCapability = AICX_CAPABILITIES_ORDER[0];
              let isIndustry = false;
              const coreQ = AICX_CORE_QUESTIONS.find(
              ...
              if (coreQ) {
                ...
                isIndustry = true;
                capability = AICX_CAPABILITIES_ORDER[0];
              }
        ...
        console.error("AI for CX chat error:", err);
        setMessages((prev) => [
          ...prev,
          {
            id: `error-${Date.now()}`,
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/aicx/chat/confirm", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          assessmentId,
      ...
      window.location.href = `/aicx/results/${data.shareId || shareId}`;
    } catch (err) {
      console.error("AI for CX confirm error:", err);
      setConfirming(false);
    }
  };
  ...
  const handleUpdateScore = (questionId: string, score: number) => {
    setScores((prev) => {
      ...
      if (existing) {
        ...
        const q = AICX_CORE_QUESTIONS.find((q) => String(q.id) === questionId);
        next.set(questionId, {
          ...
          capability: q?.capability || AICX_CAPABILITIES_ORDER[0],
