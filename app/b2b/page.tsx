...
function b2bProjectToRow(p: Raw): Row {
  return {
    key: `p-${p.id}`,
    kind: "project",
    name: p.client_name as string,
    href: `/b2b/project/${p.share_id}`,
    label: "Workshop",
...
function b2bAssessmentToRow(a: Raw): Row {
  return {
    key: `a-${a.id}`,
    kind: "assessment",
    ...
    href:
      a.status === "completed"
        ? `/b2b/results/${a.share_id}`
        : `/b2b/assessment/resume/${a.share_id}`,
    label: "Assessment",
...
function buildTree(projects: Raw[], assessments: Raw[]): Row[] {
  const projectRows = projects.map(b2bProjectToRow);
  const byId = new Map<string, Row>();
  projectRows.forEach((r, i) => byId.set(projects[i].id as string, r));

  const orphans: Row[] = [];
  for (const a of assessments) {
    const child = b2bAssessmentToRow(a);
...
export default function B2bHomePage() {
  ...
  const loadAssessments = async (email: string) => {
    ...
    try {
      const [aMineRes, aAdminRes, pMineRes, pAdminRes] = await Promise.all([
        fetch(`/api/b2b/assessments?repEmail=${encodeURIComponent(email)}`),
        fetch("/api/b2b/assessments?admin=1"),
        fetch(`/api/b2b/projects?email=${encodeURIComponent(email)}`),
        fetch("/api/b2b/projects?admin=1"),
      ]);
  ...
  return (
    <div className="min-h-screen font-m2 bg-m2-surface-light">
      <DiagnosticHeader current="b2b" user={user} onSignOut={handleSignOut} />
