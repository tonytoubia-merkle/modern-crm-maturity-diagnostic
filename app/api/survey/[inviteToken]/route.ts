...
export async function GET(
  ...
  try {
    ...
    // Look up stakeholder by invite token
    ...
    // Get existing responses if assessment already started
    ...
    const mapped = (responses as Array<Record<string, string | number | boolean | null>>).map((r) => ({
      questionId: /^\d+$/.test(String(r.question_id)) ? Number(r.question_id) : r.question_id,
      score: r.score,
    ...
    console.error("GET /api/survey/[inviteToken] error:", err);
    return NextResponse.json(
