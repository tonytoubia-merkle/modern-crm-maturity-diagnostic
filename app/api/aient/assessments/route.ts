...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("POST /api/aient/assessments error:", err);
    return NextResponse.json(
      { error: "Failed to create AI for Enterprise assessment" },
      { status: 500 }
    );
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("GET /api/aient/assessments error:", err);
    return NextResponse.json(
      { error: "Failed to fetch AI for Enterprise assessments" },
      { status: 500 }
    );
  }
}
