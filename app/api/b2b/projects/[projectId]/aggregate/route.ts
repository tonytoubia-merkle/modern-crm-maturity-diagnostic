...
import { aggregateB2bStakeholderResponses } from "@/lib/b2b/workshop/aggregation";
import {
...
import { buildB2bWorkshopAgenda } from "@/lib/b2b/workshop/agendaBuilder";
...
export async function POST(
  ...
  try {
    ...
    const allResponses: B2bResponseItem[][] = [];
        ...
        allResponses.push(
          rawResponses.map((r) => ({
            questionId: /^\d+$/.test(r.question_id)
              ...
              : r.question_id,
            score: r.score,
            capability: r.capability as B2bCapability,
    ...
    const { averagedResponses, varianceByQuestion, responseCountByQuestion } =
      aggregateB2bStakeholderResponses(allResponses);

    const capabilityScores = computeB2bCapabilityScores(averagedResponses);
    const overallScore = computeB2bOverallScore(capabilityScores);
    const maturityStage = computeB2bMaturityStage(overallScore);
    const opportunities = getB2bTriggeredOpportunities(capabilityScores);

    const agenda = buildB2bWorkshopAgenda(
    ...
    console.error("POST /api/b2b/projects/[id]/aggregate error:", err);
    return NextResponse.json(
