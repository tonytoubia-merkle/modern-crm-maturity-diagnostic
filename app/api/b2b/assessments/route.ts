...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("POST /api/b2b/assessments error:", err);
    return NextResponse.json(
      { error: "Failed to create B2B assessment" },
      { status: 500 }
    );
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("GET /api/b2b/assessments error:", err);
    return NextResponse.json(
      { error: "Failed to fetch B2B assessments" },
      { status: 500 }
    );
  }
}
