...
export default function B2bChatAssessmentPage() {
  ...
  const [industry, setIndustry] = useState<AicxIndustry | "none" | "">("");
          ...
          <form onSubmit={handleStart} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {(Object.entries(AICX_INDUSTRY_LABELS) as [string, string][]).map(
                  ([key, label]) => (
                    <button
                      key={key}
                      type="button"
                      onClick={() => setIndustry(key as AicxIndustry)}
                      className={`px-3 py-1.5 rounded-full border-2 text-xs font-medium transition-all ${
                        industry === key
                          ? "border-blue-600 bg-blue-50 text-blue-700"
                          : "border-slate-200 bg-white text-slate-700 hover:border-blue-300"
