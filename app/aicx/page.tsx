...
function aicxProjectToRow(p: Raw): Row {
  return {
    key: `p-${p.id}`,
    kind: "project",
    name: p.client_name as string,
    href: `/aicx/project/${p.share_id}`,
    label: "Workshop",
...
function aicxAssessmentToRow(a: Raw): Row {
  return {
    key: `a-${a.id}`,
    kind: "assessment",
    ...
    href:
      a.status === "completed"
        ? `/aicx/results/${a.share_id}`
        : `/aicx/assessment/resume/${a.share_id}`,
    label: "Assessment",
...
function buildTree(projects: Raw[], assessments: Raw[]): Row[] {
  const projectRows = projects.map(aicxProjectToRow);
  const byId = new Map<string, Row>();
  projectRows.forEach((r, i) => byId.set(projects[i].id as string, r));

  const orphans: Row[] = [];
  for (const a of assessments) {
    const child = aicxAssessmentToRow(a);
...
export default function AicxHomePage() {
  ...
  const loadAssessments = async (email: string) => {
    ...
    try {
      const [aMineRes, aAdminRes, pMineRes, pAdminRes] = await Promise.all([
        fetch(`/api/aicx/assessments?repEmail=${encodeURIComponent(email)}`),
        fetch("/api/aicx/assessments?admin=1"),
        fetch(`/api/aicx/projects?email=${encodeURIComponent(email)}`),
        fetch("/api/aicx/projects?admin=1"),
      ]);
  ...
  return (
    <div className="min-h-screen font-m2 bg-m2-surface-light">
      <DiagnosticHeader current="aicx" user={user} onSignOut={handleSignOut} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 mb-12">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600 leading-relaxed">
              ...
              opportunities across discoverability, agentic experience,
              adaptive personalization, and EXO measurement.
            </p>
          </div>
          <div>
            <h3 className="text-sm font-bold text-slate-900 mb-1.5">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
            <h3 className="text-sm font-bold text-slate-900">
              {scope === "all"
                ? "All AI for CX Projects"
                : "Your AI for CX Projects"}
            </h3>
            {isAicxAdmin && (
