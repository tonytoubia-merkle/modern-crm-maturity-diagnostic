...
export default async function ResumeAientAssessmentPage({
  params,
}: {
  ...
  if (assessment.status !== "completed") {
    for (let i = 0; i < AIENT_CAPABILITIES_ORDER.length; i++) {
      const cap = AIENT_CAPABILITIES_ORDER[i];
      const capQuestions = AIENT_QUESTIONS_BY_CAPABILITY[cap];
  ...
  const industry = (assessment.industry as AientIndustry | null) ?? null;
