import type {
  AientWorkshopAgenda,
  AientWorkshopAgendaSection,
  AientCapability,
} from "@/lib/aient/types";
import { AIENT_VIGNETTES } from "@/lib/aient/data/vignettes";
import { AIENT_OPPORTUNITIES } from "@/lib/aient/data/opportunities";

/**
 * AI for Enterprise workshop agenda builder. Mirrors the B2B / CSC
 * logic. Produces an agenda from a list of triggered opportunity IDs:
 ...
 *      that set, deduped by capability.
 *   3. Wrap with opening + capability deep-dives + prioritisation + close.
 */
export function buildAientWorkshopAgenda(
  triggeredOpportunityIds: string[],
  _industry?: string
): AientWorkshopAgenda {
  const targetCapabilities = new Set<AientCapability>();
  for (const oppId of triggeredOpportunityIds) {
    const opp = AIENT_OPPORTUNITIES.find((o) => o.id === oppId);
    if (opp) {
      for (const cap of opp.capabilities) targetCapabilities.add(cap);
    }
  }

  const seenCapabilities = new Set<AientCapability>();
  const matched = AIENT_VIGNETTES.filter((v) => {
  ...
  const format: AientWorkshopAgenda["format"] =
    deepDiveCount <= 2 ? "half_day" : deepDiveCount <= 4 ? "full_day" : "two_day";

  const sections: AientWorkshopAgendaSection[] = [
    {
  ...
  if (matched.length > 0) {
    sections.push(
      ...matched.map<AientWorkshopAgendaSection>((v) => ({
        title: v.title,
