...
export interface CscAggregationResult {
  averagedResponses: CscResponseItem[];
...
export function aggregateCscStakeholderResponses(
  allResponses: CscResponseItem[][]
): CscAggregationResult {
  const byQuestion: Record<
    string,
    { scores: number[]; capability: CscCapability; isIndustry: boolean }
  > = {};
  ...
  const averagedResponses: CscResponseItem[] = [];
